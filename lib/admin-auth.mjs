 ...
 *    password read from the ADMIN_BOOTSTRAP_PASSWORD env var, then proceed
 *    with normal compare. If neither a stored hash nor the env var exists,
 *    we fail closed (no login is possible) rather than seeding a guessable
 *    default. Set ADMIN_BOOTSTRAP_PASSWORD once, log in, then change it via
 *    /api/admin/auth/change-password and you can remove the env var.
...
async function ensureHash() {
  ...
  // No password has ever been set. Seed from the env bootstrap password if
  // present; otherwise fail closed — we never fall back to a hardcoded default.
  const bootstrap = process.env.ADMIN_BOOTSTRAP_PASSWORD;
  if (!bootstrap || bootstrap.length < 8) {
    throw new Error(
      'Admin password not configured. Set ADMIN_BOOTSTRAP_PASSWORD (min 8 chars) for the first login.'
    );
  }
  const hash = await bcrypt.hash(bootstrap, BCRYPT_ROUNDS);
...
export function signSession() {
  return jwt.sign({ admin: true }, jwtSecret(), { expiresIn: `${SESSION_DAYS}d`, algorithm: 'HS256' });
}
